...
// TODO 数据库名：RFInitDB-SAVE (作用：当点击保存数据后会将数据保存到这个数据库中的表中 。 （该数据库中有6个表，分别对应每一个模式。）)
public class RfInitDatabaseSave extends SQLiteOpenHelper {
    private static  final String DATABASE_NAME = "RFInitDB-SAVE";
    ...
    private  static  RfInitDatabaseSave dbHelder = null;


    private  RfInitDatabaseSave (Context context){
        super(context,DATABASE_NAME,null,1);
    }
    public static  RfInitDatabaseSave getRfInitDatabase(Context context){
        if(dbHelder == null){
            dbHelder = new RfInitDatabaseSave(context);
        }
        return dbHelder;
    ...
    //修改初始化数据。
    public long updateRfData(int mode ,int part,WorkIData updateRfData){
        String tableName;
        switch (mode){
            case 1:
                tableName = TABLE_NAME_HIIT;
        ...
        contentValues.put("P12",updateRfData.getP1());
        contentValues.put("P34",updateRfData.getP2());
        contentValues.put("RF12",updateRfData.getRf1());
        contentValues.put("RF34",updateRfData.getRf2());
        contentValues.put("F1",updateRfData.getF1());
        contentValues.put("F2",updateRfData.getF2());
        contentValues.put("F3",updateRfData.getF3());
        System.out.println("id  == "+part +"   workIData  ===  "+updateRfData.toString()+" p "+part+"  tableName "+tableName );
